...
void ApplyTheme()
{
   switch(extTheme)
   {
      case THEME_NEON_BLUE:
         ...
         gClrBorder  = C'20,50,80';      // Neon blue/grey border
         break;
      case THEME_MATRIX:
         gClrBg      = C'0,10,0';
         gClrHdr     = C'0,30,0';
         ...
         gClrDanger  = clrRed;
         gClrBorder  = C'0,40,0';        // Matrix green border
         break;
      case THEME_PURE_DARK:
         ...
         gClrBorder  = C'35,35,35';      // Pure dark border
         break;
      ...
      default:
         ...
         gClrBorder  = COLOR_BORDER;     // Onyx gold border
         break;
   }
}
...
{
   int midY      = y + ROW_H / 2;  // Vertical center of row (true centering)
   int dataStart = 90;
   int colW      = (Panel_Width - dataStart) / 4;
   
   // Label column (left-aligned, vertically centered)
   CreateLabel(prefix + "_lbl", X_Offset + 20, midY, label, gClrLabel, 9, FONT_MAIN, ANCHOR_LEFT);
   
   // 4 Value Columns (Horizontally AND Vertically centered within column)
...
void CreateDashboardTab(int y)
{
   ...
   CreateLabel(PREFIX + "Title", centerX, y + 22, "GT", gClrAccent, 11, FONT_MAIN, ANCHOR_CENTER);
   y += 50;
   ...
   // Draw table background grid (horizontal and vertical lines)
   int y_start = y;
